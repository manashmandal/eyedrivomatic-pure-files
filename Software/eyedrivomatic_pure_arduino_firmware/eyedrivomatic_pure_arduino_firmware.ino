...
int ySpeedTrim = 0; // a variable for trimming the y servo speed during normal forwards movement 
int overCorrect = 3; // Two variables used as part of the over correction system, used to ensure 
int overCorrectStatus = 0; // that the joystick centres properly after a left or right movement.
// overCorrectStatus = 0 - not centred recently, 1 = just centred 
...
boolean manoeuvreToggle = false; // By default the y servo deflection does not increase with
// global speed increases, as the x servo deflection does. This is to allow gentle course 
// correction at speed during normal driving conditions. The manoeuvring toggle equalises 
// the deflection of both servos for efficient manoeuvring. 
int manoeuvreState; // a variable for sending the state of the manoeuvre toggle to the pc
int lastDirButtPress; // a variable for the most recent direction button to be pressed 
  ...
  yservo.write(yMIDpos);// sends the y servo to it's mid position 
    ...
    if (eyedrive != EYEDRIVE) {
      ...
      EEPROM.write(0, 199);
        ...
        manoeuvreState = EEPROM.read(1); 
        speedState = EEPROM.read(2); 
        safetyBypassState = EEPROM.read(3); 
        lastDirButtPress = EEPROM.read(4); 
        ...
        overCorrect = EEPROM.read(15); 
        ySpeedTrim = EEPROM.read(16); 
}

durationTimeNS = (durationTimeNSstate * 100); 
durationTimeEW = (durationTimeEWstate * 100); 
...
if (manoeuvreState == 1){
  manoeuvreToggle = true; 
}
else {
  manoeuvreToggle = false; 
}
...
void loop() {
  ...
  //relevant servo to it's mid position after over correcting a little to ensure perfect
  //centring.
  ...
  if (driveSwitchStateEW == 1){
    ...
    if (overCorrectStatus == 0){
      if (lastDirButtPress == 1 || lastDirButtPress == 7 || lastDirButtPress == 8){
        if (overCorrectStatus == 0){
         yservo.write(yMIDpos - overCorrect);
         delay(100); 
      ...
      if (lastDirButtPress == 3 || lastDirButtPress == 4 || lastDirButtPress == 5){
        if (overCorrectStatus == 0){
         yservo.write(yMIDpos + overCorrect);
         delay(100); 
         yservo.write (yMIDpos); 
  ...
  //the serial event function governs what happens when the arduino receives an instruction from the pc 
      ...
      switch(receivedUSB) { // sorts the received instructions 
       ...
       case 33: //if the pc sends a byte to request information 
        ...
        Serial.print(",");
        Serial.print(durationTimeEWstate); 
        Serial.print(",");
        Serial.print(overCorrect); 
        Serial.print(",");
        Serial.println(ySpeedTrim); //the last data has to be "println"  
        break; 
       ...
       case 45:// safety bypass toggle (this allows repeated button presses )
        ...
        break;
       case 46: //manoeuvring toggle 
        manoeuvreToggle = !manoeuvreToggle; 
        break;
       case 49:  //stop 
       ...
       case 72: //duration ew = 5
        durationTimeEW = 5000; 
        break;
       case 73: // ySpeedTrim up one degree
       ySpeedTrim = ySpeedTrim +1; 
        break;
       case 74: // ySpeedTrim down one degree 
       if (ySpeedTrim > 0){
       ySpeedTrim = ySpeedTrim -1; 
       }
        break;
       case 75: // 
        EEPROM.write(1, manoeuvreState); 
        EEPROM.write(2, speedState); 
        EEPROM.write(3, safetyBypassState); 
        EEPROM.write(4, lastDirButtPress); 
        ...
        EEPROM.write(14, durationTimeEWstate); 
        EEPROM.write(15, overCorrect); 
        EEPROM.write(16, ySpeedTrim); 
        break;
       case 59: //overCorrect up one degree
       overCorrect = (overCorrect + 1); 
        break;
       case 76: //overCorrect down one degree
       if (overCorrect > 0){
       overCorrect = (overCorrect - 1); 
       }
        break;
