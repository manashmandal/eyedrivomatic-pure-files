...
int data14;    //data15 received from Arduino overCorrect 
int data15;    //data16 received from Arduino ySpeedTrim
...
void setup (){ 
  ...
  frame.setTitle("Eyedrivomatic Pure 4");
...
if (data0 == 0){
    PImage notManPic = loadImage("8-2.png"); 
    image(notManPic,332,116); // icon for not manoeuvring 
}
else {
    PImage manPic = loadImage("8-3.png"); 
    image(manPic,332,116); // icon for manoeuvring 
}
if ((data4 == 0) && (data3 == 0)){
...
if (data12 == 2){
  ...
  text("Manoeuvring Status", 150, 175);
  text("X Servo Centre Point           Degrees", 850, 55);
  text("Over Correct Trim                 Degrees", 850, 95);
  text("Y Servo Speed Trim             Degrees", 850, 135);
  ...
  text((data15), 1023, 135);// y servo speed trim information  
  text((data14), 1023, 95);// Over Correct trim information   
...
void keyPressed(){ 
  ...
  switch (key) {
      ...
      break;
      case 'm':
      Port.write(46); // manoeuvring toggle 
      break;
      case 'n':
